...
const h2: React.CSSProperties = { fontSize: '15px', fontWeight: 800, color: '#0F172A', margin: '0 0 4px' }
const sub: React.CSSProperties = { fontSize: '12px', color: '#64748B', margin: '0 0 14px' }

function Chip({ children, c = '#6366F1', bg = '#EEF2FF' }: { children: React.ReactNode; c?: string; bg?: string }) {
...
function SourceCard({ icon, name, what, color, n }: { icon: string; name: string; what: string; color: string; n?: number }) {
  return (
    <div style={{ flex: '1 1 180px', border: `2px solid ${color}`, background: `${color}0D`, borderRadius: '10px', padding: '12px 14px' }}>
      ...
      {n != null && <div style={{ fontSize: '20px', fontWeight: 800, color: '#0F172A', fontVariantNumeric: 'tabular-nums' }}>{n.toLocaleString()}</div>}
      <div style={{ fontSize: '11px', color: '#64748B', marginTop: '2px' }}>{what}</div>
    </div>
  )
}
...
export function GuideContent({ inline = false }: { inline?: boolean }) {
  ...
  return (
    <div style={{ maxWidth: '1100px' }}>
      <h1 style={{ fontSize: '22px', fontWeight: 800, color: '#0F172A', margin: '0 0 2px' }}>Clientbase — your campaign cockpit 🐾</h1>
      <p style={{ fontSize: '13px', color: '#64748B', margin: '0 0 20px', maxWidth: '820px' }}>
      ...
      <div style={{ ...card, marginBottom: '18px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '10px' }}>
          {[
            ['📣 Marketing Initiatives', '#6366F1', 'START HERE. Ranked campaign opportunities built from real demand — exactly what to run next, to whom, with which product.'],
            ...
            ['📊 Demand Breakdown', '#166534', 'Every product ever transacted — by clients, revenue and clinic-vs-online split. The product intelligence behind the initiatives.'],
            ...
            <div key={t} style={{ border: `1px solid ${c}33`, background: `${c}0A`, borderRadius: '8px', padding: '11px 13px' }}>
              ...
              <div style={{ fontSize: '11px', color: '#475569', marginTop: '3px' }}>{d}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ ...card, marginBottom: '18px', borderColor: '#C7D2FE', background: '#F5F7FF' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '12px' }}>
          <Chip c="#0F172A" bg="#F1F5F9">① the target pool (buy in-clinic, not online)</Chip>
          <Chip c="#166534" bg="#DCFCE7">② products to feature</Chip>
          <Chip c="#92400E" bg="#FEF3C7">③ reach by channel (✉/💬/📣)</Chip>
          <Chip c="#15803D" bg="#F0FDF4">④ the suggested play</Chip>
          <Chip c="#6366F1" bg="#EEF2FF">⑤ Build this audience →</Chip>
        </div>
      ...
      <div style={{ ...card, marginBottom: '18px' }}>
        ...
        <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
          ...
          <SourceCard icon="🛒" name="PetProject (Shopify)" color="#166534" n={cu.online} what="The website — lifetime online orders, LTV, products & SKUs back to 2019." />
        </div>

        <div style={{ textAlign: 'center', color: '#CBD5E1', fontSize: '20px', margin: '6px 0' }}>↓ matched by email / phone ↓</div>

        <div style={{ background: '#EEF2FF', border: '2px solid #6366F1', borderRadius: '10px', padding: '10px 14px', textAlign: 'center', marginBottom: '12px' }}>
          <span style={{ fontSize: '13px', fontWeight: 800, color: '#3730A3' }}>= ONE unified customer record</span>
          {s?.customers != null && <span style={{ fontSize: '13px', color: '#64748B' }}> · {s.customers.toLocaleString()} active customers (newsletter-only leads hidden by default)</span>}
        </div>

        <div style={{ fontSize: '11px', fontWeight: 700, color: '#64748B', textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: '6px' }}>+ enriched with</div>
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
          <Chip c="#92400E" bg="#FEF3C7">✉ Klaviyo — email lists, consent, last engagement</Chip>
          <Chip c="#166534" bg="#DCFCE7">💬 ChatArchitect — WhatsApp opt-in list</Chip>
          <Chip c="#DB2777" bg="#FCE7F3">🎧 Slack (WhatsApp mirror) — who’s contacting CS + sentiment</Chip>
          <Chip c="#B91C1C" bg="#FEE2E2">📦 Shopify — unfulfilled orders</Chip>
        </div>
      </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(310px, 1fr))', gap: '10px' }}>
          {[
            ...
            <div key={f.t} style={{ border: '1px solid #E2E8F0', borderRadius: '8px', padding: '10px 12px' }}>
              <div style={{ fontSize: '13px', fontWeight: 800, color: '#6366F1' }}>{f.t}</div>
              <div style={{ fontSize: '11px', color: '#475569', margin: '3px 0 6px' }}>{f.d}</div>
              <Chip>{f.e}</Chip>
            </div>
          ))}
        </div>
      ...
      <div style={{ ...card }}>
        ...
        {[
          { goal: '🔁 Win back lapsed clinic patients', why: 'Patients who bought Rx at Dr Hugh but have gone quiet — push them to re-order online.',
            chips: [['Customers: Dr Hugh', '#6B21A8', '#F3E8FF'], ['Demand: Rx Diets + Meds', '#6366F1', '#EEF2FF'], ['No purchase since 2025-06', '#B45309', '#FEF3C7']], out: 'Export → Meta + Klaviyo reactivation' },
          { goal: '🎯 Retarget the PSG 2020–21 Rx audience', why: 'In 2020–21 PetProject sold prescription meds in partnership with another clinic — those orders carry “PSG” in the Shopify SKU. A proven, high-value Rx buyer base to win back and to seed Meta lookalikes.',
            chips: [['Customers: PSG 2020-21', '#7C3AED', '#F3E8FF'], ['Demand: Meds', '#6366F1', '#EEF2FF']], out: 'Export → Meta custom + lookalike' },
          { goal: '🛒 Cross-sell a clinic product online', why: 'They buy online AND got Apoquel at the clinic — nudge them to buy it from us too.',
            chips: [['Demand: Apoquel', '#6366F1', '#EEF2FF'], ['Customers: Website', '#166534', '#DCFCE7']], out: 'Export → Meta “buy Apoquel online”' },
          { goal: '💬 WhatsApp the customers who are waiting', why: 'Paid but unfulfilled, and opted in to WhatsApp — send a proactive “sorry, running late”.',
            chips: [['Operations: Unfulfilled', '#B91C1C', '#FEE2E2'], ['Consents: WhatsApp-able', '#166534', '#DCFCE7']], out: 'Export → ChatArchitect blast' },
          { goal: '🚫 Don’t double-message', why: 'About to send an email? Exclude anyone who just got the abandoned-cart flow so it doesn’t read as spam.',
            chips: [['CRM Marketing: Abandoned cart', '#0E7490', '#ECFEFF'], ['(use as an exclusion)', '#64748B', '#F1F5F9']], out: 'Filter / exclude before sending' },
        ].map((r, i) => (
          <div key={i} style={{ display: 'flex', gap: '14px', alignItems: 'flex-start', padding: '11px 0', borderTop: i ? '1px solid #F1F5F9' : 'none' }}>
            <div style={{ flex: '0 0 230px' }}>
              <div style={{ fontSize: '13px', fontWeight: 700, color: '#0F172A' }}>{r.goal}</div>
              <div style={{ fontSize: '11px', color: '#64748B', marginTop: '2px' }}>{r.why}</div>
            </div>
            <div style={{ flex: 1, display: 'flex', gap: '6px', flexWrap: 'wrap', alignItems: 'center' }}>
              {r.chips.map(([t, c, bg], j) => (
                <span key={j} style={{ display: 'inline-flex', alignItems: 'center', gap: '6px' }}>
                  {j > 0 && <span style={{ color: '#CBD5E1', fontSize: '11px' }}>+</span>}
                  <Chip c={c} bg={bg}>{t}</Chip>
                </span>
              ))}
              <span style={{ color: '#CBD5E1', fontSize: '12px' }}>→</span>
              <span style={{ fontSize: '11px', fontWeight: 700, color: '#15803D' }}>{r.out}</span>
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={{ ...card, marginTop: '18px' }}>
        ...
        <div style={{ fontSize: '12px', color: '#475569', lineHeight: 1.6 }}>
          ...
          <b>Exceptions:</b> <Chip c="#64748B" bg="#F1F5F9">LIST - ALL - MASTER</Chip> (the unconsented catch-all);
          <Chip c="#7C3AED" bg="#F3E8FF">FLOW - AUTOSHIP - E1…E7</Chip> (subscribers consent by buying, no list needed).
          <span style={{ color: '#94A3B8' }}> Convention set in Klaviyo; the tool mirrors it. Raw Klaviyo IDs that can’t be renamed are shown as-is.</span>
        </div>
      </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '8px' }}>
          {[
            ...
            <div key={t} style={{ border: '1px solid #E2E8F0', borderRadius: '8px', padding: '8px 11px' }}>
              <div style={{ fontSize: '12px', fontWeight: 700, color: '#0F172A' }}>{t}</div>
      ...
      {!inline && <div style={{ textAlign: 'center', margin: '22px 0 6px' }}>
        <a href="/clients" style={{ fontSize: '14px', fontWeight: 700, color: 'white', background: '#6366F1', borderRadius: '8px', padding: '10px 22px', textDecoration: 'none' }}>Open the Clientbase →</a>
      </div>}
    </div>
  )
}
