  ...
  return (
    <div style={{ maxWidth: '820px' }}>
      <div style={{ marginBottom: '18px', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '12px' }}>
        <div>
          <h1 style={{ fontSize: '20px', fontWeight: 700, color: C.ink }}>Categories</h1>
        ...
        {canEdit && !adding && <button onClick={() => setAdding(true)} style={{ background: C.indigo, color: 'white', border: 'none', borderRadius: '7px', padding: '8px 16px', fontSize: '13px', fontWeight: 600, cursor: 'pointer', whiteSpace: 'nowrap' }}>+ Add category</button>}
        {!canEdit && <span style={{ fontSize: '11px', color: C.faint, background: C.monoBg, padding: '5px 10px', borderRadius: '6px', whiteSpace: 'nowrap' }}>View only · Admin to edit</span>}
      </div>
      ...
      {!loading && !error && (
        <div style={{ background: 'white', border: '1px solid #E2E8F0', borderRadius: '8px', overflow: 'hidden' }}>
          <div style={{ display: 'grid', gridTemplateColumns: cols, gap: '10px', background: C.wash, borderBottom: '1px solid #E2E8F0', padding: '10px 16px' }}>
            {['Category', 'GP Floor', 'Storage', 'Channel', 'SKU #', ''].map((h, i) => (
              <span key={i} style={{ fontSize: '10px', fontWeight: 700, color: C.faint, textTransform: 'uppercase', letterSpacing: '0.06em' }}>{h}</span>
            ))}
          </div>

          {adding && (
            <div style={{ display: 'grid', gridTemplateColumns: cols, gap: '10px', padding: '10px 16px', alignItems: 'center', background: C.primaryBg, borderBottom: '1px solid #E2E8F0' }}>
              ...
              <div style={{ display: 'flex', gap: '6px' }}>
                ...
                <button onClick={() => setAdding(false)} style={{ background: 'none', border: '1px solid #E2E8F0', borderRadius: '5px', padding: '5px 8px', fontSize: '12px', cursor: 'pointer', color: C.muted }}>✕</button>
              </div>
            </div>
          )}
          ...
          {rules.map((r, i) => {
            ...
            return (
              <div key={r.category} style={{ display: 'grid', gridTemplateColumns: cols, gap: '10px', padding: '11px 16px', alignItems: 'center', borderBottom: i < rules.length - 1 ? '1px solid #F1F5F9' : 'none', background: i % 2 === 0 ? 'white' : '#FAFAFA' }}>
                <span style={{ fontSize: '12px', fontWeight: 700, color: C.ink }}>{r.category}</span>
                {isEdit ? (
                  <>
                    ...
                    <div style={{ display: 'flex', gap: '6px' }}>
                      <button onClick={() => saveEdit(r.category)} disabled={busy} style={{ background: C.indigo, color: 'white', border: 'none', borderRadius: '5px', padding: '5px 10px', fontSize: '12px', fontWeight: 600, cursor: 'pointer' }}>Save</button>
                      <button onClick={() => setEditing(null)} style={{ background: 'none', border: '1px solid #E2E8F0', borderRadius: '5px', padding: '5px 8px', fontSize: '12px', cursor: 'pointer', color: C.muted }}>✕</button>
                    </div>
                  </>
                ) : (
                  <>
                    <span style={{ fontSize: '15px', fontWeight: 700, color: C.ink }}>{r.gp_floor > 0 ? `${Math.round(r.gp_floor * 100)}%` : '—'}</span>
                    <span style={{ fontSize: '12px', color: C.sub }}>{r.storage_rule || '—'}</span>
                    <span style={{ fontSize: '12px', color: C.muted }}>{r.channel_restriction || 'All'}</span>
                    <span style={{ fontSize: '13px', fontWeight: 700, color: C.indigoInk, fontFamily: 'ui-monospace, monospace' }}>{r.sku_digit ?? '—'}</span>
                    <div style={{ display: 'flex', gap: '8px' }}>
                      {canEdit ? (
                        <>
                          <button onClick={() => { setEditing(r.category); setDraft({ ...r }) }} style={{ background: 'none', border: '1px solid #E2E8F0', borderRadius: '5px', padding: '4px 10px', fontSize: '11px', fontWeight: 600, color: C.sub, cursor: 'pointer' }}>Edit</button>
                          <button onClick={() => del(r.category)} style={{ background: 'none', border: 'none', fontSize: '11px', fontWeight: 600, color: C.redInk, cursor: 'pointer' }}>Delete</button>
                        </>
                      ) : <span style={{ fontSize: '11px', color: C.knobOff }}>—</span>}
                    </div>
                  </>
                )}
              </div>
