...
const FLAG_STYLE: Record<Flag, { label: string; bg: string; color: string; border: string }> = {
  below_floor: { label: 'Below floor',  bg: '#FEF2F2', color: '#991B1B', border: '#FECACA' },
  borderline:  { label: 'Borderline',   bg: '#FFFBEB', color: '#92400E', border: '#FCD34D' },
  ok:          { label: 'Approved',     bg: '#F0FDF4', color: '#166534', border: '#BBF7D0' },
}
...
function LogicLayerPage() {
  ...
  function SortArrow({ col }: { col: typeof sortCol }) {
    if (sortCol !== col) return <span style={{ color: '#CBD5E1', marginLeft: '3px' }}>↕</span>
    return <span style={{ color: '#6366F1', marginLeft: '3px' }}>{sortAsc ? '↑' : '↓'}</span>
  }
  ...
  if (loading) {
    return <div style={{ padding: '60px', textAlign: 'center', color: '#94A3B8', fontSize: '13px' }}>Loading…</div>
  }
  ...
  return (
    <div style={{ maxWidth: '1100px' }}>
      ...
      <div style={{ marginBottom: '20px' }}>
        <h1 style={{ fontSize: '20px', fontWeight: 700, color: '#0F172A' }}>Logic Layer</h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: '10px', marginBottom: '20px' }}>
        {catSummary.map(s => s && (
          <div key={s.cat} style={{ background: 'white', border: '1px solid #E2E8F0', borderRadius: '8px', padding: '12px 14px' }}>
            <p style={{ fontSize: '11px', fontWeight: 700, color: '#64748B', marginBottom: '2px' }}>{s.cat}</p>
            <p style={{ fontSize: '18px', fontWeight: 700, color: '#0F172A', marginBottom: '6px' }}>
              {(s.floor * 100).toFixed(0)}% floor
            </p>
            <div style={{ display: 'flex', gap: '6px', fontSize: '11px', flexWrap: 'wrap' }}>
              {s.below > 0 && <span style={{ background: '#FEF2F2', color: '#991B1B', padding: '1px 6px', borderRadius: '4px', fontWeight: 600 }}>{s.below} below</span>}
              {s.borderline > 0 && <span style={{ background: '#FFFBEB', color: '#92400E', padding: '1px 6px', borderRadius: '4px', fontWeight: 600 }}>{s.borderline} edge</span>}
              <span style={{ background: '#F0FDF4', color: '#166534', padding: '1px 6px', borderRadius: '4px', fontWeight: 600 }}>{s.ok} ok</span>
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '12px', marginBottom: '20px' }}>
        {[
          { label: 'Below Floor',  value: counts.below,      bg: '#FEF2F2', border: '#FECACA', color: '#991B1B', flag: 'below_floor' as Flag | 'All' },
          { label: 'Borderline',   value: counts.borderline,  bg: '#FFFBEB', border: '#FCD34D', color: '#92400E', flag: 'borderline' as Flag | 'All' },
          { label: 'Approved',     value: counts.ok,          bg: '#F0FDF4', border: '#BBF7D0', color: '#166534', flag: 'ok' as Flag | 'All' },
          { label: 'No Price Data',value: counts.no_price,    bg: '#F8FAFC', border: '#E2E8F0', color: '#64748B', flag: 'All' as Flag | 'All' },
        ].map(card => (
          <button
            key={card.label}
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '14px', alignItems: 'center', flexWrap: 'wrap' }}>
        <select
          ...
          style={{ fontSize: '12px', border: '1px solid #E2E8F0', borderRadius: '6px', padding: '6px 10px', background: 'white', color: '#0F172A' }}
        >
        ...
        {(catFilter !== 'All' || flagFilter !== 'All') && (
          <button
            ...
            style={{ fontSize: '11px', color: '#6366F1', background: 'none', border: 'none', cursor: 'pointer', padding: '0 4px' }}
          >
            Clear filters ×
          </button>
        )}
      ...
      <div style={{ background: 'white', border: '1px solid #E2E8F0', borderRadius: '8px', overflow: 'hidden' }}>
        {/* Header */}
        <div style={{ display: 'grid', gridTemplateColumns: '90px 1fr 120px 90px 90px 90px 80px', gap: '0', background: '#F8FAFC', borderBottom: '1px solid #E2E8F0' }}>
          {[
            { label: 'SKU',       col: null },
            ...
            <div
              ...
              style={{ padding: '9px 12px', fontSize: '10px', fontWeight: 700, color: '#94A3B8', textTransform: 'uppercase', letterSpacing: '0.06em', cursor: col ? 'pointer' : 'default', userSelect: 'none' }}
            >
          ...
          return (
            ...
            >
              <div style={{ padding: '10px 12px' }}>
                <Link to={`/items/${skuToPath(p.sku_code)}` as never} style={{ fontSize: '11px', color: '#6366F1', textDecoration: 'none', fontWeight: 600, fontVariantNumeric: 'tabular-nums' }}>
                  {p.sku_code}
                </Link>
              </div>
              <div style={{ padding: '10px 12px' }}>
                <p style={{ fontSize: '12px', fontWeight: 500, color: '#0F172A', lineHeight: 1.3 }}>{p.name}</p>
                {p.brand && <p style={{ fontSize: '11px', color: '#94A3B8', marginTop: '1px' }}>{p.brand}</p>}
              </div>
              <div style={{ padding: '10px 12px' }}>
                <span style={{ fontSize: '11px', color: '#64748B' }}>{p.category}</span>
              </div>
              <div style={{ padding: '10px 12px', fontVariantNumeric: 'tabular-nums' }}>
                <span style={{ fontSize: '12px', fontWeight: 600, color: '#64748B' }}>{(floor * 100).toFixed(0)}%</span>
              </div>
              <div style={{ padding: '10px 12px', fontVariantNumeric: 'tabular-nums' }}>
                {worstGp !== null
                  ? <span style={{ fontSize: '13px', fontWeight: 700, color: worstGp < floor ? '#EF4444' : worstGp < floor + GP_BAND ? '#F59E0B' : '#166534' }}>
              ...
              <div style={{ padding: '10px 12px', fontVariantNumeric: 'tabular-nums' }}>
                {bestGp !== null
                  ? <span style={{ fontSize: '12px', color: '#94A3B8' }}>{(bestGp * 100).toFixed(1)}%</span>
                  : <span style={{ fontSize: '12px', color: '#CBD5E1' }}>—</span>}
              </div>
              <div style={{ padding: '10px 12px' }}>
